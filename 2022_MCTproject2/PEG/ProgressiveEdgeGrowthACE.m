...
parent = zeros(N + M, 1, 'uint16');%Record the relationships in the tree.classic_PEG.exe -numM 252 -numN 504 -codeName 20.dat -degFileName DenEvl_20.deg

for v = 1 : N
    if mod(v, ceil(N/20)) == 1
        disp(['PEG LDPC under Construction. Now ' num2str(v/N*100) '%.']);
    end
    for k = 1 : 500%for each k, we add an '1' in H.
